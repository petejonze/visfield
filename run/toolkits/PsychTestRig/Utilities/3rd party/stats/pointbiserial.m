function [r,h,p,ci,stats] = pointbiserial(d,x,alpha,tail)
...
if(isempty(tail))
    [h,p,ci,stats] = ttest2(x(d),x(~d));
elseif(strcmp(tail,'np'))
    [p,h,ci,stats] = ranksum(x(d),x(~d),alpha);    
else
    [h,p,ci,stats] = ttest2(x(d),x(~d),alpha,tail);    
end