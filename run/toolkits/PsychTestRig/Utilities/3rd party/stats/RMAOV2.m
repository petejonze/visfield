function [P, table, eta2, pEta2] = RMAOV2(X,alpha,printOutput,IVnames)
...
%
%
% Modified by Pete R Jones, 1st Oct 2012
...
if nargin < 4 || isempty(IVnames)
    IVnames = {'IV1', 'IV2'};
end
if ~(iscellstr(IVnames) && length(IVnames)==2)
    error('IVnames should be a 2 item cellstr');
end
...
% make stats table
table = {};
table(1,:) = {'SOV'                          	,'SS'   ,'df'   ,'MS'   ,'F'    ,'P'};
table(2,:) = {'Subjects'                       	,SSS    ,v7     ,MSS    ,F4     ,P(4)};
table(3,:) = {sprintf('IV1: %s', IVnames{1})  	,SSA    ,v1     ,MSA    ,F1     ,P(1)};
table(4,:) = {sprintf('Error(%s)',IVnames{1})  	,SSEA   ,v2     ,MSEA   ,[]     ,[]};
table(5,:) = {sprintf('IV2: %s', IVnames{2})  	,SSB    ,v3     ,MSB    ,F2     ,P(2)};
table(6,:) = {sprintf('Error(%s)',IVnames{2}) 	,SSEB   ,v4     ,MSEB   ,[]     ,[]};
table(7,:) = {'IV1xIV2'                         ,SSAB   ,v5     ,MSAB   ,F3     ,P(3)};
table(8,:) = {'Error(IV1xIV2)'                  ,SSEAB  ,v6     ,MSEAB  ,[]     ,[]};
table(9,:) = {'[Error'                      	,SSSE   ,v8     ,MSSE   ,[]     ,[]};
table(10,:)= {'Total'                         	,SSTO   ,v9     ,[]     ,[]     ,[]};

% calc effect sizes
eta2 = [SSA/SSTO, SSB/SSTO, SSAB/SSTO];
pEta2 = [SSA/(SSA+SSEA), SSB/(SSA+SSEB), SSAB/(SSA+SSEAB)];
