function handles = barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type, fontsizes)
...
elseif nargin == 11
	legend_type = 'plot';
elseif nargin == 13 && ~isempty(fontsizes) % pete
    fontsize_small = fontsizes(1);
    fontsize_large = fontsizes(2);
end
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
		handles.legend = legend(bw_legend, 'location', 'best', 'fontsize', fontsize_small);
		legend boxoff;
	else
		handles.legend = [];
	end
	...
	for i = 1:numbars
		...
		x = mean(x([1 3],:));
        if ndims(errors) == 3
            handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,1,i), errors(:,2,i), 'k', 'linestyle', 'none', 'linewidth', 2);
            ymax = max([ymax; barvalues(:,i)+errors(:,2,i)]);
            ymin = min([ymin; barvalues(:,i)-errors(:,1,i)]);
        else
	...
	ylim([ymin*1.1 ymax*1.1]); %pete: changed from [0 ymax*1.1]
	...
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			...
			for j = 1:length(xdata)
				text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 60, 'fontsize', fontsize_small, 'HorizontalAlignment', 'right');
			end
		end
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontsize', fontsize_large);
	end
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontsize', fontsize_large);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontsize', fontsize_large);
	end
	
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', fontsize_small, 'xtick',1:numgroups, 'linewidth', 1,'xgrid','off','ygrid','off'); %pete: changed linewidth 2=>1
